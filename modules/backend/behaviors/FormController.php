...
class FormController extends ControllerBehavior
{
    ...
    public function create_onSave($context = null)
    {
        ...
        if ($model->table == 'ibang_tas_schedules') {
            $dataForm = $this->formWidget->getSaveData();
            for ($i=0; $i < sizeof($dataForm['time']); $i++) {
                $datas[$i]['time'] = $dataForm['time'][$i];
                $datas[$i]['day'] = $dataForm['day'];
                $datas[$i]['grade_id'] = $dataForm['grade'];
                $datas[$i]['subject_id'] = $dataForm['subject'];
                $datas[$i]['staff_id'] = $dataForm['staff'];
            }
            $model->insert($datas);
        } else {
